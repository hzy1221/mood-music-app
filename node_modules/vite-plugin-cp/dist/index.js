...
async function doCopy(config, cwd, globOptions, copyOptions) {
  ...
  dest = vpRuntimeHelper.toAbsolutePath(dest, cwd);
  const cpFile = makeCopy(transform);
  const run = async (source) => {
    const absSource = vpRuntimeHelper.toAbsolutePath(source, cwd);
    let isNotFlatten = false;
    try {
      isNotFlatten = node_fs.statSync(absSource).isDirectory() && !flatten;
    } catch (e) {
    }
    const matchedPaths = await tinyglobby.glob(absSource, globOptions);
    ...
    await Promise.all(matchedPaths.map((matchedPath) => {
      ...
      let destDir = dest;
      if (isNotFlatten) {
        const tmp = node_path.parse(node_path.relative(absSource, matchedPath));
        targetFileName = tmp.base;
        destDir = node_path.join(destDir, tmp.dir);
      } else {
        targetFileName = node_path.parse(matchedPath).base;
      }
      const destPath = node_path.join(destDir, changeName(targetFileName, rename));
...
function pluginCp(opts) {
  ...
  if (enableBanner) {
    vpRuntimeHelper.banner(PLUGIN_NAME);
  }
  if (logLevel) {
  ...
  plugin[hook] = async function() {
    ...
    if (delay !== void 0) {
      await vpRuntimeHelper.sleep(delay);
    }
    called = true;
    const startTime = Date.now();
    await Promise.all(targets.map((target) => {
      if (!isWhatType.isObject(target)) {
